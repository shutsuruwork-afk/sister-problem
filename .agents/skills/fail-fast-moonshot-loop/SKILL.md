...
**世代ごとに 1 枚。次の生成は、この地図に対して行う。** 実体は `math/SEARCH_MAP.md`。
設計は MAP-Elites（Mouret & Clune 2015, *Illuminating search spaces by mapping elites*）に倣い、
**最適解を 1 つ探すのではなく、探索空間を照らす地図そのものを成果物として持つ。**

**資源 × 手法の格子**とし、各セルは 3 状態のいずれかを取る。

| 状態 | 意味 |
| :--- | :--- |
| **エリート** | そのセルで現在最良の採用結果。**1 セル 1 件**（これが重複採用を構造的に防ぐ） |
| **壁** | そのセルは閉じている。**確定時点のエリート構成を併記**（波及確認で再検証候補になる） |
| **空** | まだ触れていない。**空セルがそのまま「開いている方向」である** |

**詳細は `references/search-map.md` を読むこと。** Phase 1 で生成する前と、Phase 6 で
判定を地図へ反映する前に必要になる。格子の初期構成、壁を書く粒度（CDCL の nogood 原則）、
地図を成長させないための忘却の規律、壁が崩れる条件が、そこにある。
...
## 実寸表 (Production-Scale Sheet)

**世代ごとに 1 枚、本番規模での実寸を出す。** 検証は $n \le 8$ で行われるため、放っておくと
本番規模の数字が一度も現れない。実際、453 件の実験のうち試験 $n$ の中央値は 8 で、
律速のある $n=28$ に触れたものは 1 件も無かった。

### これは計算結果ではなく、投影である

$a(28)$ を計算する必要はない。**現在のエリート構成が $n=28$ で何を意味するかは、
閉形式と算術だけで出る。** 状態数もメモリ所要量も、実行せずに確定する。

**各行に必ず「定義 / 実測 / 外挿」のどれかを付ける。** 投影を結果として読ませないための最小の措置である。

### 表の形

| 項目 | 値 | 由来 | 下限との比較 |
| :--- | ---: | :--- | :--- |
| 状態数（DP が保持） | $1.489 \times 10^{12}$ | 定義（閉形式） | — |
| メモリ / 層 / 素数 | 477 GiB | **エリート構成から再計算** | 下限 173 GiB の 2.76 倍 ✓ |
| 予算 2,013 GiB に対する余裕 | 4.2x | 定義 | — |
| 総遷移回数 | $3.3 \times 10^{15}$ / 素数 | 外挿 | — |
| 推定所要 | 約 110,000 コア時 | 外挿 | — |
| **実測アンカー** | 現行ハードで載る最大 $n$ での実測 | **実測** | — |
| アンカーから $n=28$ への外挿係数 | $2.86^{(28-n)}$ | 定義 | — |

### 3 つの規則

1. **毎回エリート構成から再計算する。前世代の値を持ち越さない。** これが最も重要である。
   再計算すればメモリは毎回 477 GiB になり、**それと矛盾する主張はその場で見える。**
   持ち越して倍率を積むと見えない。実際、積算だけで「437 兆倍削減」に到達し、
   誰も $n=28$ での絶対値を計算し直さなかった。
2. **実測アンカーを 1 行は必ず置く。** 現行ハードに載る最大の $n$（現状 $n=22\sim24$ 前後）で
   実際に走らせた値を入れる。**全行が外挿の表は、算術の反復であって測定ではない。**
   ROADMAP が「最初の仕事」と名指ししている 11-bit 版のスループット実測が、ここに入る。
3. **どれか 1 行でも理論的下限を割ったら、エリート構成が壊れている。** 止めて報告する。
   下限との比較列は、そのための欄である。

### 律速の表との関係

**実寸表が、「律速の把握」の表を更新する手段である。** 世代ごとに実寸を出せば、
律速の表は自動的に最新になる。逆に、実寸表を出さない世代の律速の表は、前世代の判断の
コピーでしかない。
...
graph TD
    ...
    R -- "Yes" --> P7["Phase 7: 世代交代<br/>実寸表を 1 枚出す / 残りを破棄"]
    P7 --> P1
    R -- "No" --> P3
```
...
| **7. 世代交代** | 検証キューを使い切った | **「実寸表」を 1 枚出す**（エリート構成から再計算、実測アンカー 1 行必須）。**未検証で残ったものを含め全て破棄**し、**地図の空セルを数えて**世代番号を進める。空セルが減っていないのに生成だけ続いた世代が 2 回続いたら、仮説の質ではなく**軸の取り方**を変える段階である | 実寸表 ＋ 更新された地図 ＋ 空セル数 ＋ 次の世代番号 | 実寸表が出て、Phase 1 へ戻る |
