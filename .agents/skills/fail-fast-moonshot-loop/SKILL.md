...
**1 世代 = 50 件生成 → 全件を順位付け → 上位 10 件だけを検証 → 残り 40 件は破棄。**
次の世代はまた 50 件を新しく作る。**残った 40 件を持ち越して再スコアリングしてはならない。**
...
**落選 40 件に理由を書く必要はない。** 「11 位だった」は証拠ではなく順位の言い換えである。
PRUNED の理由が価値を持つのは、それが**検証して得た結果**だからであって、
順位の記録には同じ価値がない。書けば記録が増えるだけで、判断材料は増えない。

#### 枠は検証数にだけ掛ける

**上位 10 件のうち何件を ADOPT するかに枠を設けてはならない。** 0 件のこともある。
採否は規律 5 の証拠が決める。ここに枠を掛けると通過率を製造することになり、
「50 件中 10 件」という枠の目的（相互比較による重複の発見）そのものを裏切る。

50 件を並べて順位を付ける作業自体に価値がある。**孤立して 1 件ずつ判定していたために、
...
**世代ごとに 1 枚。次の 50 件は、この地図に対して生成する。** 地図が無ければ、
生成は毎回ゼロからになり、既に壁だと分かっている方向へ繰り返し歩く。

`math/SEARCH_MAP.md` に次の 4 節で記録する。

### 1. 確定した壁（根本原因のクラスタ）

検証して駄目だったものを、**個別の仮説ではなく原因でまとめる**。同じ壁に 2 件以上ぶつかったら
1 つのクラスタにする。各クラスタに「壁の内容 / 確定させた証拠 / ここで死んだ仮説」を書く。

現時点で確定しているクラスタの例:

| 壁 | 証拠 | 死んだ仮説 |
| :--- | :--- | :--- |
| 行走査の切断長が既に最小 | 反対角走査は $3^n \to 3^{2n}$（NOTES §4.4） | H-07, H-09 |
| 境界次元・ポート数の爆発 | 象限境界ポートが $B(n)^2$（NOTES §4） | H-06, H-08, H-19 |
| 下位ビットしか決まらない | 局所合同式は下位 2〜3 bit 止まり | H-14, H-16 |
| 状態数は削れない | 有用/到達 比が $n$ とともに 1 へ（NOTES §4.1） | 枝刈り系すべて |
| 母関数は Non-D-finite | H-13 の検証 | D-finite 前提の全仮説 |

### 2. 理論的下限

「理論的下限」節の表を参照する。**生成の時点で下限を割る提案は書かない。**

### 3. 現在の律速

「律速の把握」節の表を参照する。**A 級を狙う提案は、この表の最小余裕の資源を削るものに限る。**

### 4. まだ開いている方向

壁でも下限でもない領域を明示する。**50 件はここから生成する。**
ここが空欄なら、それは「新しい方向が要る」という信号であって、
無理に 50 件を埋める合図ではない。

### 地図の使い方

- **生成時**: 提案が §1 の壁に当たるなら、**その壁への回答を併記できない限り書かない。**
- **判定時**: PRUNE したら §1 に反映する。既存クラスタに入るなら新項目を作らず、そのクラスタに追記する。
- **世代の切り替え時**: §4 を書き直す。ここが更新できない世代が 2 回続いたら、
  仮説の質ではなく問題の見方を変える段階に来ている。
...
- (d) 世代交代で 50 件を新しく生成したとき（Phase 1）
...
```mermaid
graph TD
    P1["Phase 1: 律速の表を更新 & 地図に対して 50 件生成"] --> P2["Phase 2: 全 50 件を採点し順位付け<br/>上位 10 件だけを検証キューへ / 残り 40 件は破棄"]
...
| :--- | :--- | :--- | :--- | :--- |
| **1. 創出** | ループ開始時、または Phase 7 から | **まず「律速の把握」の表を実測から更新する。** 次に**探索地図に対して** 50 件を生成する。地図 §1 の壁に当たる提案は、その壁への回答を併記できない限り書かない。下限を割る提案も書かない | 更新された律速の表 ＋ 50 件の仮説（ID・名称・狙う倍率とその根拠） | 律速が数値で確定し、50 件が出そろった |
| **2. 順位付け** | 50 件が出そろった | **50 件を並べて** $S$ 式で採点し降順に並べる。並べた状態で**重複を探す** — 同じ資源を削る提案、同じ集合を篩う提案は、隣り合えば見える。上位 10 件を検証キューへ、**残り 40 件は破棄** | 検証キュー 10 件 ＋ 重複の指摘 | 10 件が確定し、40 件が消えた |
...
| **6. 記録** | 判定が確定した | 検証キューから外し、記録ファイルを更新。**PRUNED なら探索地図 §1 に反映**（既存クラスタに入るなら追記、新しい壁ならクラスタを作る）。ADOPTED なら Bonus 検証の要否を判断して記録 | 更新された記録ファイル ＋ 更新された地図 | 記録と地図が判定を反映している |
| **7. 世代交代** | 検証キュー 10 件を使い切った | **未検証で残ったものを含め全て破棄**し、地図 §4（開いている方向）を書き直して世代番号を進める。§4 を更新できない世代が 2 回続いたら、仮説の質ではなく問題の見方を変える段階である | 更新された地図 ＋ 次の世代番号 | Phase 1 へ戻る |
...
ただし例外が 1 つある。**探索地図 §4「まだ開いている方向」を更新できない世代が 2 回続いたら止めて報告する。** 生成する先が無い状態で 50 件を埋めると、地図に載らない提案（＝壁でも下限でもない領域の外にあるもの）が混ざり始める。これは行き詰まりの唯一の内的な検出点である。
...
**終了時の報告に必ず含めるもの**: 回したサイクル数 / ADOPTED と PRUNED の件数と対象 / 得られた実測値の要点 / 検証キューの消化状況 / 地図 §4 の更新有無 / 次に検証すべき最上位仮説とその理由。
...
1. **仮説トラッカー**: 全仮説の順位、判定状態、切り捨て理由、採用実績を動的に更新する。冒頭に現世代番号・生成 50 件・検証キュー 10 件の消化状況を明記する。
2. **分類マスター**: **Part 1（普遍的）** と **Part 2（特定条件）** に完全分離した体系。規律 1 の裁定ルールに従って記載する。
3. **ベンチマーク台帳**: 全ブレークスルーの実測値・実行時間・スループット・検証ログの公式記録。
4. **探索地図** (`math/SEARCH_MAP.md`): 確定した壁のクラスタ・理論的下限・現在の律速・まだ開いている方向。**次世代の生成はこれに対して行う。**
