...
- **Bonus（可変・必要に応じて追加）**: **仮説を ADOPT したら、その正当性を守る Bonus 検証を 1 つ追加する。** 採用物が増えるほどスイートが厚くなる設計であり、これが回帰ゼロを維持する仕組みそのものである。

「5-Tier」は Tier 1〜5 の 5 段を指す通称である。Tier 0 は入口の静的ゲート、Bonus は採用ごとに増える可変枠なので、**スイートの総段数は固定ではない**。段数を数え間違えて「全部通った」と判断しないこと。
...
| 状態 | 意味 | キュー | 必須の記録 |
| :--- | :--- | :--- | :--- |
| `ADOPTED` | 実測で有意な改善 + 回帰ゼロ | 外す | 台帳に実測値、分類マスターに Part 1/2、Bonus 検証を 1 つ追加 |
...
| **6. 記録** | 判定が確定した | キューから除去し、記録 3 ファイルを更新。ADOPTED なら Bonus 検証を追加。前提が変わった場合のみ再順位付け | 更新された記録 3 ファイル | 3 ファイルが判定を反映している |
