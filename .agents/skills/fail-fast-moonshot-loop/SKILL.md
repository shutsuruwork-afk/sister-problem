...
- **`BLOCKED` が上位を占めても止まらない。降格して次へ進む。** 手元の環境で検証できない仮説がキューの先頭に並ぶのは順位付けの問題であって、ループの終わりではない。別リストへ移し、検証可能な次の仮説に進む。環境が整った時点で復帰させる。
...
| :--- | :--- | :--- | :--- | :--- |
| **1. 創出** | ループ開始時、または Phase 7 から | **まず「律速の把握」の表を実測から更新する。** その律速を動かす仮説を 30 件以上創出する | 更新された律速の表 ＋ 仮説リスト（ID・名称・狙う倍率とその根拠） | 律速が数値で確定し、30 件以上がトラッカーに記載された |
