...
**ADOPT のたびに、その成果が地図の他の場所に何をしたかを確認する。** これを省くと、
成果は記録されるが波及しない。**波及は双方向で、2 つは何かを殺し、2 つは何かを生き返らせる。**

Phase 6 で、次の 4 点を順に見る。地図は小さいので、再検証ではなく走査で足りる。
...
graph TD
    ...
    V1 --> P6["Phase 6: 記録と地図を更新<br/>ADOPT なら波及確認 4 点"]
    V2 --> P6
    V3 --> P6
    P6 --> S{"停止条件 / 予算到達?"}
...
| **6. 記録と波及** | 判定が確定した | 検証キューから外し、記録ファイルと地図を更新。**PRUNED ならセルを壁にする（確定時点のエリート構成を併記）／既存の壁に統合する。** ADOPTED なら Bonus 検証の要否を判断し、**「波及確認」の 4 点を走査する** | 更新された記録・地図 ＋ 波及確認の 4 行 | 記録と地図が判定を反映し、波及が確認されている |
